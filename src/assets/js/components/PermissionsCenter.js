...
export const PermissionsCenter=(mount, deps={})=>{
  ...
  function clearAuditStream(){ if(typeof unAudit==='function'){ try{ unAudit(); }catch{} unAudit=null; } }
  function renderTab(){ if(currentTab!=='audit') clearAuditStream(); const c=qs('#tabContent',ui); if(currentTab==='roles') c.replaceChildren(renderRolesTab()); else if(currentTab==='users') c.replaceChildren(renderUsersTab()); else c.replaceChildren(renderAuditTab()); const idx={roles:0,users:1,audit:2}[currentTab]; ui.querySelectorAll('.tab').forEach(b=>b.classList.remove('is-active')); ui.querySelectorAll('.tab')[idx].classList.add('is-active'); }
  ...
  function renderRolesTab(){
    ...
    const warnSA = editingSuperAdmin? el('p',{className:'warn mt-1'},['Edición de SuperAdmin bloqueada (solo lectura).']): null;
  ...
  function renderAuditTab(){ const box=el('div',{className:'mt-1'},[ el('p',{className:'text-muted'},['Últimos cambios de permisos']) ]); const list=el('div',{id:'auditList',className:'mt-1'},[]); box.append(list); clearAuditStream(); unAudit=deps.streamAuditLogs?.((items)=>{ list.replaceChildren(...items.map(it=> renderAuditItem(it))); })||null; return box; }
