...
(function init(){
  ...
  addRoute('/sedes', ()=> requireAuth(()=> guard(PERMS.MANAGE_SEDES, ()=> SedesAdmin(root))));
  addRoute('/employees', ()=> requireAuth(()=> guard(PERMS.MANAGE_EMPLOYEES, ()=> EmployeesAdmin(root))));
  addRoute('/supervisors', ()=> requireAuth(()=> guard(PERMS.MANAGE_SUPERVISORS, ()=> SupervisorsAdmin(root))));

  // Operación
  addRoute('/imports', ()=> requireAuth(()=> guard(PERMS.IMPORT_DATA, ()=> ImportHistory(root))));
  ...
  // Supervisor/Empleado
  addRoute('/upload', ()=> requireAuth(()=> guard(PERMS.UPLOAD_DATA, ()=> UploadData(root))));
